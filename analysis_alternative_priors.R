...
alternative_ar_prior <- prior(normal(0, 0.3), class = "ar")
...
pp_noar <- pp_check(hierarchical_model_log_noar, type = "dens_overlay") +
  ggtitle("PP Check Without AR(1) - Alternative Priors") +
  xlab("log(Sea Ice Extent)") + ylab("Density") +
  theme_minimal(base_size = 14)

ggsave("figures/ppc_noar_alt_model.pdf",
  pp_noar,
  width = 6,
  height = 4
)
...
hierarchical_model_log_ar1 <- brm(
  ...
  data = long_df,
  family = gaussian(),
  prior = c(alternative_priors, alternative_ar_prior),
  iter = 4000,
  warmup = 2000,
  chains = 2,
  cores = 2,
...
pp_ar1 <- pp_check(hierarchical_model_log_ar1, type = "dens_overlay") +
  ggtitle("PP Check With AR(1) - Alternative Priors") +
  xlab("log(Sea Ice Extent)") + ylab("Density") +
  theme_minimal(base_size = 14)

ggsave("figures/ppc_ar1_alt_model.pdf",
  pp_ar1,
  width = 6,
  height = 4
)
...
) + ggtitle("Trace Plots: No-AR Model (Alternative Priors)")

ggsave(
  filename = "figures/noar_trace_plots_alt.pdf",
  plot     = trace_noar,
  width    = 8,
  height   = 6
)
...
trace_ar1 <- mcmc_trace(
  ...
  pars = params_ar1,
  facet_args = list(ncol = 1)
) + ggtitle("Trace Plots: AR(1) Model (Alternative Priors)")

ggsave(
  filename = "figures/ar1_trace_plots_alt.pdf",
  plot     = trace_ar1,
  width    = 8,
  height   = 6
)
